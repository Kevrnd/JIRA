...
inwardLinks.each { link ->
    if (link.issueLinkType.id == issueLinkTypeId && link.sourceObject.summary.contains("Onboarding")) {
...
// Если задача на уже создана, добавляем в ее поле Observers пользователя который повторно запросил Onbording
    ...
    if (cf_hr.getValue(issue).toString().equals("Slot")) {
        componentIds.addAll([10301L, 10303L, 14445L, 14625L, 14626L, 10210L]) // "Jira", "Gsuite",  "Rocket.Chat.Slot", "Equipment.Slot", "VPN.Slot", "Confluence"
        ...
        componentIds.addAll([10301L, 10210L, 10302L, 10303L, 10420L]) // "Jira", "Confluence", "VPN", "Gsuite", "Rocket.Chat"
            if(cf_job_position.getValue(issue).toString() in developers_list) {
                componentIds.add(10422L) // GitLab
            }
    }
    ...
    def value_block = ComponentAccessor.optionsManager.getOptions(cfConfig)?.find {
        it.toString() == 'Добавить пользователя'
    }
